...
from main_lib import record_to_file, run_analyze, save_sound
...
class Ui_Form(object):
    ...
    def retranslateUi(self, Form):
        ...
        ___qlistwidgetitem5.setText(
            QCoreApplication.translate("Form", "Ivando Sibarani", None)
        )
    ...
    def retrain_preclick(self):
        ...
        self.retrainBtn.setText("Retraining...")

    def retrain_event(self):

        X, y = load_data()
        # split the data into training, validation and testing sets
        self.data = split_data(X, y, test_size=0.1, valid_size=0.1)
        ...
        self.early_stopping = EarlyStopping(mode="min", patience=5, restore_best_weights=True)
        # self.add_to_log()
        ...
        self.model.fit(self.data["X_train"], self.data["y_train"], epochs=self.epochs, batch_size=self.batch_size, validation_data=(self.data["X_valid"], self.data["y_valid"]),
                callbacks=[self.tensorboard, self.early_stopping])

        # save the model to a file
        self.model.save("results/model.h5")
        
        self.add_to_log(f"Evaluating the model using {len(self.data['X_test'])} samples...")
        self.loss, self.accuracy = self.model.evaluate(self.data["X_test"], self.data["y_test"], verbose=0)

        self.add_to_log(f"Loss: {self.loss:.4f}")
        self.add_to_log(f"Accuracy: {self.accuracy*100:.2f}%")

        
    def retrain_postclick(self):
        self.add_to_log("[SUCCESS] Retrain complete.")
        self.retrainBtn.setText("Retrain AI")
